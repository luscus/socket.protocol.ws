...
exports._connect = function _connect (connections) {
  ...
  socket._usePath = socket.config.usePath || false;
  ...
  connections.forEach(function connectionIterator (connection) {
    ...
    if (!socket.connections[uri]) {
      ...
      // TODO: put this in the socket.model.client
      connection.protocol = connection.parent.config.protocol;
      
      var url = tools.net.getUri(connection);

      socket.connections[uri].client = socketio.connect(url);
